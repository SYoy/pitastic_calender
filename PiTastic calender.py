...
## Load the poems list and get vari
ables from librarian.csv file
...
num_left += 10 if today =='25-12-2018'

#write update to config.csv
write_to_config([today[0],str(num_left),"nan"])
...
def print_poem( setting="poems" ):
    ...
    if setting=="poems":
        chosen_index = randrange(len(hot_poems_list))
        chosen_poem = hot_poems_list[chosen_index]
        folder="poems/"
    elif setting=="always_usable":
        chosen_index = randrange(len(always_usable_list))
    ...
    with open ("/home/pi/PiTastic calender/"+ folder + str(chosen_poem) + ".txt") as f:
        lines = f.readlines()
